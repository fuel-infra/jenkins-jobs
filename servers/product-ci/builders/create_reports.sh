...
for DISTR in centos ubuntu; do
    ...
    python license-compliance/extract.py "${DISTR}${RELEASE}${UPDATES_SUFFIX}.csv" "license_mos_${RELEASE}${UPDATES_SUFFIX}_${DISTR}.csv" "enc_lic_${DISTR}_output_${RELEASE}${UPDATES_SUFFIX}.csv" "${KEYPATH}"
done
