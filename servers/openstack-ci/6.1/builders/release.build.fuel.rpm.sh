...
build_rpm_fuel () {
    ...
    local release=`git --git-dir ${PACKAGENAME}-src/.git rev-list --no-merges HEAD | wc -l`
    ...
    [ "$GERRIT_STATUS" != "MERGED" ] && release="${release}.git`git --git-dir ${PACKAGENAME}-src/.git rev-parse --short HEAD`"
