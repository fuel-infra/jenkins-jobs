...
LATEST_TARGET_UBUNTU=$(curl -sSf "${LATEST_MIRROR_ID_URL}/mos-repos/ubuntu/9.0.target.txt" | sed '1p; d')
...
LATEST_TARGET_CENTOS=$(curl -sSf "${LATEST_MIRROR_ID_URL}/mos-repos/centos/mos9.0-centos7/os.target.txt" | sed '1p; d')
