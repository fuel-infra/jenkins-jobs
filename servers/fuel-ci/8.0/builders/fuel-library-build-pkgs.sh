...
BUILD_RPM_PACKAGES="fuel-library8.0"
BUILD_DEB_PACKAGES="fuel-library8.0"
...
for pkg in ${BUILD_RPM_PACKAGES}; do
  docker run --privileged --rm -v "${SOURCE_PATH}/${pkg}:/opt/sandbox/SOURCES" \
           ...
           -u ${UID} \
           fuel-8.0/rpmbuild_env /bin/bash /opt/sandbox/build_rpm_in_docker.sh
done
...
for pkg in ${BUILD_DEB_PACKAGES}; do
  docker run --rm -u ${UID} \
           ...
           fuel-8.0/debbuild_env /bin/bash /opt/sandbox/build_deb_in_docker.sh
done
