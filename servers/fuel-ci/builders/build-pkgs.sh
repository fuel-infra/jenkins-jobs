...
# for rpm
git archive --format=tar.gz --worktree-attributes HEAD --output="${SOURCE_PATH}/${PROJECT_PACKAGE}-${PACKAGE_VERSION}.tar.gz"
...
# for deb
cp -v "${SOURCE_PATH}/${PROJECT_PACKAGE}-${PACKAGE_VERSION}.tar.gz" "${SOURCE_PATH}/${PROJECT_PACKAGE}_${PACKAGE_VERSION}.orig.tar.gz"
mkdir -p "${SOURCE_PATH}/${PROJECT_PACKAGE}"
cp -rv "${PROJECT_ROOT}/debian" "${SOURCE_PATH}/${PROJECT_PACKAGE}"
dch -c "${SOURCE_PATH}/${PROJECT_PACKAGE}/debian/changelog" -D trusty -b --force-distribution -v "${PACKAGE_VERSION}-${RELEASE}" "${DEBMSG}"
