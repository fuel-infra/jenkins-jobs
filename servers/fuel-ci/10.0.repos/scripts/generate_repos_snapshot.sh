...
# fixme: mirror.fuel-infra.org could point to broken mirror
...
# Store snapshot for FUEL deb repo
# 9.0-2016-06-23-164100
# ^^^^^^^^^^^^^^^^^^^^^
__fuel_latest_deb_mirror_id="$(\
    curl "http://${FUEL_MIRROR_HOST}/repositories/ubuntu/snapshots/${FUEL_UBUNTU_VERSION}-latest.target.txt" \
    | head -1)"
store "FUEL_UBUNTU_MIRROR_ID" "${__fuel_latest_deb_mirror_id}"
...
# Store snapshots for full set of FUEL rpm repos

# <distribution_name>-2016-07-14-082020
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
__dt_snapshot="$(\
        curl "http://${FUEL_MIRROR_HOST}/repositories/centos/${FUEL_CENTOS_REPO_PREFIX}-centos7/snapshots/os-latest.target.txt" \
        | head -1)"
store "FUEL_CENTOS_OS_MIRROR_ID" "${__dt_snapshot}"
